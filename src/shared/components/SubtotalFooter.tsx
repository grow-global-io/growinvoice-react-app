	...
	useEffect(() => {
		if (
			...
			const calculatedTotal = parseFloat(
				(formik?.values?.sub_total - discount + taxPercentage).toFixed(2),
			);
			formik?.setFieldValue("total", calculatedTotal);
			if (formik?.values?.due_amount) {
				formik?.setFieldValue("due_amount", calculatedTotal);
			}
		} else {
			const calculatedTotal = parseFloat((formik?.values?.sub_total || 0).toFixed(2));
			formik?.setFieldValue("total", calculatedTotal);
			if (formik?.values?.due_amount) {
				formik?.setFieldValue("due_amount", calculatedTotal);
			}
